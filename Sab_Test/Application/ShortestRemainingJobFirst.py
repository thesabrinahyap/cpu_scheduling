...
def ShortestRemainingJobFirst(pInfo):
    while pInfo.plist or pInfo.queue:
        ...
        # the not pInfo.multi_feedback_check has to be there because if ShortestRemainingJobFirst is used as an algo in MultiLevelFeedbackQueue, there is a
        # chance that level 3 will be executed and plist is not empty (it has not been emptied yet because of idle
        # time), so if plist is not empty yet, ShortestRemainingJobFirst will forever compare pInfo.min_process and pInfo.plist
        ...
        if pInfo.multi_check:
            print("MULTILEVEL QUEUE CHECK: ShortestRemainingJobFirst")
            return
...
if __name__ == "__main__":
    pInfo = Process("ShortestRemainingJobFirst")
    # Customization
